...
\title{Why its is Hard to Fabricate Monsters\\ \LARGE Towards the Automation of Conceptual Blending}
...
%\title{Introducing Ontological Blending%On Blending Principles in Ontology - 6 pages max%Unification% tertium comparationis From Eclectic Alignments to Creative Blends%:On Blending Principles in Ontology%Poems, Cardinals, and Categories% Design%The Way We Blend%
%\thanks{Full Instructions and template files can be downloaded from\\ http://www.iospress.nl/authco/instruction\_crc.html.}
%}
%\subtitle{From Eclectic Alignments to Creative Blends}%Diversity vs. Unification: On Blending Principles in Ontology}% Design}


%\author{Paper Nr. 433 \\ Keywords: Ontologies \& Ontology Languages, AI \& Creativity}
<<<<<<< .mine
\author{
\institute{Institute for Knowledge and Language Engineering,  Otto-von-Guericke University of Magdeburg. Corresponding author: okutz@ovgu.de}}
=======
...
The theory of conceptual blending has been applied very successfully  in cognitive
 science to explain the process of concept generation. In previous work we have shown
  an formalization and implementation of  of conceptual blending that is  borrowing techniques 
  ...
  our attempts (and similar approaches in the literature) have been reconstructive;
   ...
   by computationally blending of two carefully selected input spaces (e.g, a house and a boat) 
   and a proper base. This paper describes an attempt to take the next step and automate the
    concept generation based on a database of input spaces.
...
Conceptual
blending in the spirit of \citet{FauconnierTurner2003} operates by
...
`imaginative' configurations. A classic example for this is the
...
Goguen defines an approach that he terms \emph{algebraic semiotics} in 
 which certain structural aspects of semiotic systems are logically
  formalized in terms of algebraic theories, sign systems, and their mappings \citep{Goguen99}. 
%
In \cite{goguenharrell09}, algebraic
semiotics has been applied to user interface design and conceptual blending.
 ...
 -- indeed, Goguen and Harrell admit that many  aspects of blending, in particular concerning
  ...
  cannot be captured formally. \citep{goguenharrell09} However, the structural
aspects \emph{can} be formalized and provide insights into the space of possible blends. 
The formalization of these blends can be formulated  using the algebraic specification language OBJ3 \citep{GoguenMalcolm96}. 


In \citet{Kutz2014} we have presented an approach to computational conceptual blending, 
which is in the tradition of Guguen's proposal. In the paper we suggest to represent the input 
spaces as ontologies (e.g., in the OWL Web Ontology Language\footnote{With `\OWL' we refer
\ to \OWL 2 DL, see \url{http://www.w3.org/TR/owl2-overview/}}). In the paper we presented how
 the Distributed Ontology Language (DOL) can be used to specify conceptual blends with the help
  of \emph{blending diagrams}. These diagram encode the relationships between the base space 
  ...
  an ontology repository which allows users to manage 
  and collaborative work on ontologies.  DOL, Hets, and Ontohub provide a powerful set of tools, 
  which make it easy to specify and computationally execute conceptual blends. 

In this paper we will discuss how we utilized DOL, Hets, and Ontohub in an attempt to build 
a prototype system that automates concept invention. The goal is to make the step from a
 ...
 one concept (e.g., houseboat) with the help of some carefully selected input spaces
  ...
  from a repository in Ontohub and attempts to blend them in a way that meets some given requirements. 
  In our experiment we use a repository of ontologies about animals as input spaces and try to blend
   ...
   of our approach and how we simplified in in our prototype implementation.  Afterward we discuss our initial results. 
...
We follow a variation of Goguen's approach to computational conceptual blending,
 which has been proposed by Marco Schorlemmer (see Figure \ref{blendingModel}. In the next sections we discuss the various elements. 
\subsection{(Weakened) Input Spaces}
 In this model there are two (or more) input spaces $I1$ and $I2$, which are in our 
 case represented as OWL ontologies.  These ontologies randomly selected from a repository 
 of animal ontologies in Ontohub. These ontologies are not `finetuned' for a particular blend, 
...
\end{figure}
 ...
 \begin{figure*}[htbp]
\begin{center}
\includegraphics[width=0.6 \textwidth]{blendModel.pdf}
\caption{The Schorlemmer model for computational blending}
\label{blendingModel}
\end{center}
\end{figure*}

 
Schorlemmer's proposal differs from \cite{goguenharrell09} by introducing an extra step:
 the ontologies  $I1$ and $I2$ are not blended directly, but are first weakened to two theories 
 $I1^*$ and $I2^*$.  There are different strategies that can be used to generate  the weaker theories
  ...
  that is undesired for the blend. We will discuss it in more detail below in the context of evaluation. 
...
The base ontology is identifying some structure that is shared across $I1^*$ and $I2^*$.
 Or, to put it differently, the base ontology contains some theory, which can be found in
  both the input spaces, but it abstracts from the peculiarities of the input spaces and generalizes the theory in some domain-independent way. 
...
The ontologies  $I1^*$ and $I2^*$ together with the base ontology and the two
  ...
  Informally, what happens is that blendoid is a disjoint union of $I1^*$ and $I2^*$, 
  where the shared structure from  the base is identified.\footnote{Technically, the 
  blendoid is the co-limit of the underlying diagram. For the formal details see \cite{}.}
...
While not explicitly mentioned by Schorlemmer, the background knowledge 
and requirements play an essential role in his model.  Figure \ref{blendingModel} 
represents a static view of how two input spaces are blended. However, since there are a
 vast number of potential blends, most of which are poor, computational concept blending is an iterative process.
  In each cycle, a new blendoid is created, its is evaluated against background knowledge and requirements, and,
   if the blendoid is rejected, the next cycle is started with a different weakened input spaces and/or a different base.
    Ideally, the results of the evaluation is supposed to guide the changes in the next cycle.  
    We will discuss the role of evaluation in more detail in section \ref{eval}.
...
In order to automate the blending process as modeled in Figure \ref{blendingModel}, 
we had to  provide the following functionality: given two selected ontologies from the 
repository, repeat the following steps until the blend is successful: (i) weaken the input 
space and generate $I1^*$ and $I2^*$, (ii) create the base ontology and the
 interpretations that link the base to $I1^*$ and $I2^*$,  (iii) execute the blend 
 and generate the blendoid, (iv) evaluate the blend.  
...
\paragraph{Weakening the input space.} For the purpose of the initial implementation
 we wrote a simple script that removes some axioms in an OWL file.
  The script preserves the declaration of classes, individuals, and properties
   ...
   The selection of the axioms that are deleted is randomized.
...
For the purpose of an initial implementation we are currently working with a very
 simplified approach, where the bases consist basically of the shared signature
  of $I1^*$ and $I2^*$, which allows for trivial interpretations from the base to the  
  The only exception is the class Monster, which is mapped to the animals within
   the input spaces (e.g., Monster in the base ontology is mapped to $I1^*$ and $I2^*$. 
   Tiger and to Viper in the input ontologies.)\footnote{This approach presupposes that the 
   same terminology is used consistently across the animal ontologies. However, since
  the ontologies were all developed in-house, this was not an issue.} -- While
 ...
 like a tiger with a forked tongue and scales, but no tigers with a viper-like tail, 
 because this would require the base to identify the tail of the tiger with the snake. 

\paragraph{Running the Blend}
 Hets provides the capability to run the blend. Technically, this is a colimit computation, 
 a construction that abstracts the operation of disjoint unions modulo the identification 
 of certain parts specified in by the base and the interpretations. \cite{??}
  
\paragraph{Evaluation}  \label{eval}
 Hets integrates a number of theorem prover and consistency checkers. 
 We used both FaCT and Darvin for the evaluation of the blendoids. 
...
we use to DOL to specify a new ontology that combines a blendoid with the background knowledge. 
This combined ontology is evaluated for logically consistency via Hets.  
The reason why the background knowledge is essential here is that   the detection of problems 
often requires more information that is contained within the blendoid.
 To return to our example about tigers and vipers, assume  $I1^*$ 
 contains the information that tigers have four claws,  and $I2^*$ contains
  the information that vipers have no appendages. In this case, the resulting
   blend will be a leg-less monster with claws. Without the additional background 
   knowledge that claws are appendages, it is impossible for a consistency checker to detect the inconsistency.  
...
both input spaces. In other words, if the information in the blendoid is contained
 ...
 that one of the ontologies is weakened so far that it does not contribute   
 anything significant to the blendoid. We used Fact to check for that. 

Often a blending process is done with certain requirements in mind. E.g., 
in our example we may look for monsters that are have four appendages. 
...
Our  prototype implementation works in the sense that the system creates monsters by blending two animals.
 ...
 (ii) the weakening of the input spaces does not utilize the results of the evaluation, but happens randomly. 

\marginpar{INCLUDE FUTURE WORK HERE}


% Future work
%In the future we are planning to use heuristic-driven theory projection (HDTP) for the generation of the base ontology based on $I1^*$ and $I2^*$.\citet{} HDTP  applies anti-unification to generalize two input theories to a more general theory
 %So far  HDTP has been implemented for FOL, to make it work in our context we are going to need  to support OWL.  
...
Interestingly, the limitations of the results are not (or only indirectly) caused
 by the fixing of the base space or the randomized weakening of the import spaces. 
...
of the blendoids that are generated by our system.
 ...
 and what shape it has. Both input spaces contained these information 
...
information that is left over from the shark ontology.

This issue would be partially addressed by choosing  a weakening strategy 
that utilizes the results of the evaluation process to selectively remove axioms 
(see below). However, the more general point is that humans expect a description of a monster
to answer certain information -- like "What does it look like?"
...
in spite of the shortcomings of some of it components. However, to get blendoids
...
they perform blends. Without that information the system cannot 
...
A classic example for this is the blending of the concepts \emph{house} and \emph{boat}, yielding as most straightforward blends the concepts of a \emph{houseboat} and a \emph{boathouse}, but also an \emph{amphibious vehicle} \cite{goguenharrell09}.
...
%of type casting in OBJ3 used in \cite{goguenharrell09} for formalising
%methaphors like the personification of a boat.

In \cite{goguenharrell09}, conceptual blending is formalised categorically, focusing on the structural aspects of the blending process. In the following, we adapt this approach to ontological engineering.
...
Fox Harrell \cite{goguenharrell09}, algebraic
...
%
For instance, in the \con{House} and \con{Boat} blending formalised in \cite{goguenharrell09}, there are, in our terminology, 48 blendoids over a fixed base diagram that are axiom preserving, commutative   and closed.\footnote{Note that this differs from the (slightly inconsistent) terminology in \cite{goguenharrell09}.}
...
Define \defsty{semiotic system} (\cite{goguenharrell09}) = algebraic theory (declaration of sorts and operations) + \defsty{level ordering} on sorts (with a top sort) + priority ordering on sorts at each level.
...
%As Goguen and Harrell \cite{goguenharrell09} point out, in a blending, not all properties need be preserved. In our case, the views into BoatHouse do not preserve all properties. As  Resident$\_$Boat is mapped to User, the views require  Resident$\_$Boat to be a subclass of Person, which is in fact not the case. Its reason is that the personification of the boat as a person is made available by the view, but it is not present in the target ontology. This is appropriate here, as this personification is a (local) blending principle that should not find its ways into the ontology.
